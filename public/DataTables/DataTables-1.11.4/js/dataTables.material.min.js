...
$jscomp.findInternal = function (a, c, b) {
    ...
    for (var e = a.length, d = 0; d < e; d++) {
        var m = a[d];
        if (c.call(b, m, d, a)) return {i: d, v: m}
    }
    return {i: -1, v: void 0}
};
$jscomp.ASSUME_ES5 = !1;
...
(function (a) {
    ...
    function d() {
        for (var f = a("table.mdc-data-table__table").children(), g = 0; g < f.length; g++) if ("THEAD" === f[g].tagName) {
            ...
            console.log(n);
            for (var h = 0; h < n.length; h++) if ("TR" ===
                n[h].tagName) {
                a(n[h]).addClass("mdc-data-table__header-row");
                for (var k = a(n[h]).children(), p = 0; p < k.length; p++) "TH" === k[p].tagName && a(k[p]).addClass("mdc-data-table__header-cell")
            }
        } else if ("TBODY" === f[g].tagName) for (a(f[g]).addClass("mdc-data-table__content"), n = a(f[g]).children(), h = 0; h < n.length; h++) if ("TR" === n[h].tagName) for (a(n[h]).addClass("mdc-data-table__row"), k = a(n[h]).children(), p = 0; p < k.length; p++) "TD" === k[p].tagName && a(k[p]).addClass("mdc-data-table__cell");
        console.log(f)
    }

    var m = a.fn.dataTable;
    a.extend(!0, m.defaults, {
        dom: "<'mdc-layout-grid'<'mdc-layout-grid__inner'<'mdc-cell mdc-layout-grid__cell--span-6'l><'mdc-cell mdc-layout-grid__cell--span-6'f>>><'mdc-layout-grid dt-table'<'mdc-layout-grid__inner'<'mdc-cell mdc-layout-grid__cell--span-12'tr>>><'mdc-layout-grid'<'mdc-layout-grid__inner'<'mdc-cell mdc-layout-grid__cell--span-4'i><'mdc-cell mdc-layout-grid__cell--span-8'p>>>",
        renderer: "material"
    });
    ...
    m.ext.renderer.pageButton.material = function (f, g, n, h, k, p) {
        var y = new m.Api(f), t = f.oLanguage.oPaginate, E = f.oLanguage.oAria.paginate || {}, q, r, z = 0,
            C = function (w, A) {
                var B, F = function (u) {
                    u.preventDefault();
        ...
        try {
            var D = a(g).find(b.activeElement).data("dt-idx")
        } catch (w) {
        }
        C(a(g).empty().html('<div class="pagination"/>').children(), h);
        D !== e && a(g).find("[data-dt-idx=" + D + "]").trigger("focus")
    };
    a(b).on("init.dt", function (f, g) {
        "dt" === f.namespace && (new a.fn.dataTable.Api(g), d())
    });
    a(b).on("draw.dt",
        function (f, g) {
            "dt" === f.namespace && (new a.fn.dataTable.Api(g), d())
